...
//   const AnalyticsScreen({super.key, required String userId});
//
//   @override
...
//           .limit(10);
//
//       final totalCorrect = answers.where((a) => a['iscorrect'] == 1).length;
...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  Future<void> _fetchAnalytics() async {
    ...
    try {
      ...
      // FIXED: Use "answeredAt" (camelCase) instead of "answeredat" (lowercase)
      final recentAnswers = await _supabase
          .from('user_answers')
          .select()
          .order('"answeredAt"', ascending: false)  // ✅ CORRECT - quoted camelCase
          .limit(10);
      ...
      for (var a in recentAnswers) {
        recentActivity.add({
          'question': 'Question ${a['mcqId']}',  // FIXED: camelCase
          'category': 'General',
          'is_correct': (a['isCorrect'] ?? 0) == 1,  // FIXED: camelCase
          'time_spent': a['timeSpentSeconds'] ?? 0,  // FIXED: camelCase
          'answered_at': a['answeredAt'] ?? '',  // FIXED: camelCase
        });
      }
  ...
  Future<List<Map<String, dynamic>>> _calculateCategoryStatsManually() async {
    try {
      ...
      for (var mcq in mcqs) {
        ...
        final mcqProg = progress.firstWhere(
              (p) => p['mcqId'] == id,  // FIXED: camelCase
          orElse: () => {},
        );

        if (mcqProg.isNotEmpty) {
          final attempts = (mcqProg['timesAttempted'] as int?) ?? 0;  // FIXED: camelCase
          final correct = (mcqProg['timesCorrect'] as int?) ?? 0;  // FIXED: camelCase
